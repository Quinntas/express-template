...
export abstract class Repo<Domain extends DomainType<any>, Table extends MySqlTable> {
    ...
    protected constructor(cfg: RepoConfig<Domain>) {
        ...
        this.redisClient = cfg.redisClient;
    }
    ...
    async select(config: SelectOptions): Promise<Result<UnknownObject[], RepoError>> {
        ...
        if (config.cachingOptions) await this.redisClient.set(config.cachingOptions.key, JSON.stringify(res), config.cachingOptions.expires);
