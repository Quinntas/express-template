...
import {
    PrivateLoginToken,
    PublicLoginToken,
} from '../../../../useCases/userLogin/userLogin.dto';
import {UserDecodedExpressRequest} from '../../user.decodedExpressRequest';

export async function ensureUserAuthenticatedMiddleware(
    ...
    if (tokenValue.length !== 2 || tokenValue[0] !== 'Bearer')
        return Err(new HttpError(401, 'Invalid token format'));

    const publicDecoded: PublicLoginToken = JWT.decode<PublicLoginToken>(
        tokenValue[1],
    );

    if (!publicDecoded) return Err(new HttpError(401, 'Invalid token'));

    const privateToken = await cacheService.get(
        loginRedisKeyPrefix + publicDecoded.user.pid,
    );

    if (!privateToken.ok)
        return Err(new InternalError('Could not retrieve token from redis'));

    const privateDecoded: PrivateLoginToken = JWT.decode<PrivateLoginToken>(
        privateToken.val,
    );
